 ...
 * as a value. The class manages all online (live) files on the server.
 * 
 * @author Neobi
 * 
 */
public class FileManager {
	...
	public List<Session> getAllSessions() {
		...
		Collection serverFiles = files.values();
		Iterator<ServerFile> it = serverFiles.iterator();
		while (it.hasNext()) {
			ServerFile f = it.next();
			List<Session> fileSessions = f.getAllSessions();
			if (fileSessions != null) {
				Iterator<Session> iter = fileSessions.iterator();
				while (iter.hasNext()) {
					Session s = iter.next();
